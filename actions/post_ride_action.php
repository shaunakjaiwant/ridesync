...
if ($origin === '' || $destination === '' || $travel_date === '' || $travel_time === '') {
    $_SESSION['ride_error'] = "All fields are required.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}

if (strlen($origin) > 150 || strlen($destination) > 150) {
    $_SESSION['ride_error'] = "Departure and destination must be 150 characters or fewer.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}

if (strcasecmp($origin, $destination) === 0) {
    $_SESSION['ride_error'] = "Departure and destination cannot be the same.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}

if (strtotime($travel_date) < strtotime(date('Y-m-d'))) {
    $_SESSION['ride_error'] = "Travel date cannot be in the past.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}
...
if (!$travelTimestamp || $travelTimestamp < (time() - 300)) {
    $_SESSION['ride_error'] = "Travel time cannot be in the past.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}

if ($seats_available < 1 || $seats_available > 5) {
    $_SESSION['ride_error'] = "Seats must be between 1 and 5.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}
...
if ($hasAnyMapValue && !$hasCompleteMapValue) {
    $_SESSION['ride_error'] = "Please set both departure and destination pins on the map.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}
...
if ($hasCompleteMapValue) {
    ...
    if (!$validCoordinates) {
        $_SESSION['ride_error'] = "Map coordinates are invalid. Please set the route again.";
        header("Location: /ridesync/pages/post_ride.php");
        exit();
    }
    ...
    if ($pinDistanceKm !== null && $pinDistanceKm < 0.05) {
        $_SESSION['ride_error'] = "Departure and destination pins are too close to be a valid ride.";
        header("Location: /ridesync/pages/post_ride.php");
        exit();
    }
    ...
    if ($routePolylineRaw !== '' && $routePolyline === null) {
        $_SESSION['ride_error'] = "Route path could not be verified. Clear the map and set the route again.";
        header("Location: /ridesync/pages/post_ride.php");
        exit();
    }
}
...
if (mysqli_fetch_assoc(mysqli_stmt_get_result($stmt))) {
    $_SESSION['ride_error'] = "You already posted this route at the same date and time.";
    header("Location: /ridesync/pages/post_ride.php");
    exit();
}
